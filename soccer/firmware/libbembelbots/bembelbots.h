/**
 * @file bembelbots.h
 * Declatation of shared data proved by BB.
 */

#pragma once

#include "config/botnames.h"
#include <framework/ipc/time.h>

#include <cstring>

#define CURRENT_BACKEND_VERSION 2

static constexpr int naoqiBackendPort{9559};
...
enum LBBActuatorIds {
    ...
    lbbNumOfPositionActuatorIds,

    // Hardness request
    headYawHardnessActuator = lbbNumOfPositionActuatorIds,
    ...
    lbbNumOfNormalActuatorIds,

    usActuator = lbbNumOfNormalActuatorIds,

    lbbNumOfActuatorIds
};

constexpr int lbbNumOfHardnessActuatorIds = lbbNumOfPositionActuatorIds;
constexpr int lbbNumOfLedActuatorIds = rFootLedBlueActuator + 1 -
                                   faceLedRedLeft0DegActuator;

enum BackendType {
    BackendType_Naoqi = 0,
    BackendType_Lola,
    BackendType_Sequence,
    BackendType_Simspark
};
...
public:
    BBSensorData() :
          robotName(RobotName::UNKNOWN),
          tick(0),
          staticMotionActive(false),
          backendType(BackendType_Naoqi),
          backendVersion(CURRENT_BACKEND_VERSION),
          timestamp(-1) {
        memset(sensors, 0, sizeof(sensors));
        memset(eulersTop, 0, sizeof(eulersTop));
        memset(eulersBottom, 0, sizeof(eulersBottom));
    }

    RobotName robotName;
    float sensors[lbbNumOfSensorIds];

    // misnomer, not just euler angles!
    // postition vector + euler angles
    // (x,y,z,wx,wy,wz)
    // origin is midpoint between feet/ankles projected to ground plane
    float eulersTop[6];
    float eulersBottom[6];

    int32_t tick;

    bool staticMotionActive;

    bool connected = false;

    /**
     * Type of Backend:
     * 0 = naoqi backend (real robot)
     * 1 = sequence loader backend
     * 2 = simulator backend (backend-simspark)
     */
    BackendType backendType;
    int32_t backendVersion;
    
    sysMicroTime timestamp;
};

struct BBActuatorData {
    float actuators[lbbNumOfActuatorIds];
    volatile sysMicroTime timestamp;
...
static const char *const actuatorNames[] = {
