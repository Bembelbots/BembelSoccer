...
template<typename T>
T clamp(T min_val, T val, T max_val) {
    return std::max(std::min(max_val, val), min_val);
}

template<typename T>
T minmax(T min_val, T val, T max_val) {
...
struct closer {
    ...
    bool operator()(const T &x, const T &y) const {
        return dist(x, a) < dist(y, a);
    }
};

template<class U, class T>
bool closer_to(const U &what, const T &c1, const T &c2) {
    return dist(c1, what) < dist(c2, what);
};
