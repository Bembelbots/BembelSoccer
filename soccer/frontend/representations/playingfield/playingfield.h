...
SERIALIZABLE_ENUM_CLASS(Cross,
    ...
    (L_OPP_GOALBOX_RIGHT),
    (T_OPP_GOALBACK_LEFT),
    (T_OPP_GOALBACK_RIGHT),
    (L_OPP_GOALBACK_LEFT),
    (L_OPP_GOALBACK_RIGHT),
    (T_OWN_PENALTY_LEFT),
    (T_OWN_PENALTY_RIGHT),
    (L_OWN_PENALTY_LEFT),
    (L_OWN_PENALTY_RIGHT),
...
public:
    /**
     * create playing field object from pre-defined dimensions
     * @fieldSize field size
     */
    PlayingField(FieldSize fieldSize = FieldSize::SPL); // default to SPL because of runtime contextpool

    /**
     * create playing field object from JSON file as defined by SPL rules
     * @json_file path to field dimensions.json file
     */
    PlayingField(const std::string &json_file);
    ...
    std::vector<LandmarkPole> getPoles() const;
    ...
    /**
    * optimal position for goalie to block ball
    ...
    */
    ...
    std::array<Coord, 4> getCorner();
    std::array<Coord, 4> getGoalSpot();
...
private:
    ...
    struct FieldMeasurements {
        ...
        float penaltyAreaLength{1.65f};
        float penaltyAreaWidth{4.0f};
        float goalboxAreaLength{0.6f};
        float goalboxAreaWidth{2.2f};
        float goalWidth{1.5f};
        float goalPostDiameter{0.1f};
        float goalInsideDepth{0.5f};
