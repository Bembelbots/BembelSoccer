...
TeamBall::TeamBall(int ownRobotID)
{    
    m_dbscan = DBScan();
    m_robotID = ownRobotID;
}
...
void TeamBall::update(std::array<Ball, NUM_PLAYERS> balls, 
            std::array<Robot, NUM_PLAYERS> robots, 
            float teamBallConfidenceFalloff,
            float teamBallNearestToBallTolerance,
            float teamBallDBScanEpsilon,
...
bool TeamBall::isNearestToOwnBall(){
    ...
    for (int i = 0; i < NUM_PLAYERS; i++) 
    {
        ...
        auto age = getTimestampMs() - m_currentBalls[i].timestamp;     
        if((length < minLength || minIndex == -1) and (m_currentBalls[i].timestamp > 0) and (age < 1.5f*TeamComm::defaultBcastIntervalMs)) 
        {
            minLength = length;
            minIndex = i;
        }
...
{
    TimestampMs minAge = 0;
    int minIndex = -1;

    for (int i = 0; i < NUM_PLAYERS; i++) 
    {
        TimestampMs age = getTimestampMs() - m_currentBalls[i].timestamp;     
        if((age < minAge || minIndex == -1) and (m_currentBalls[i].timestamp > 0)) 
        {
            minAge = age;
            minIndex = i;
        }
    ...
    return m_currentBalls[minIndex];
}
