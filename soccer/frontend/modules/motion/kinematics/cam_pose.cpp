...
Eigen::Matrix4f CameraPose::getHeadTransformation(joints::Sensors &s, const Eigen::Vector3f &a, float balancingFoot) {
    ...
    torsoTranslation = RotMat::homogenous(bodyTransformation.block<3,3>(0,0).transpose() * bodyTransformation.block<3,1>(0,3));
...
std::array<camPose,2> CameraPose::getPose(joints::Sensors &sensors, Eigen::Vector3f bodyAngles) {
    Foot fsr {
        {
    ...
    float balancingFoot = calcSupportFoot(fsr);
