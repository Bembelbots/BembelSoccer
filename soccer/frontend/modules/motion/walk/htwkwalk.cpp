...
#include "htwk/kinematics/InverseKinematics.h"
#include "htwk/kinematics/LegPositionPlain.h"
#include "htwk/kinematics/PositionPlain.h"
#include "htwk/utils/joints.h"
#include "htwk/utils/stl_ext.h"

using namespace std;

float HTWKWalk::max_forward = 0.28f;
float HTWKWalk::max_backward = 0.25f;
float HTWKWalk::max_strafe = 0.35f;
float HTWKWalk::max_turn = 1.7f;
float HTWKWalk::velocity_combination_damping = 0.3f;
...
float HTWKWalk::get_spline_value(float t, float* spline) {
...
float HTWKWalk::calcSupportFoot(FSR fsr) {
...
float HTWKWalk::linearParameterChange(float in, float target, float delta) {
    if (in < target) {
...
void HTWKWalk::detect_and_proceed_phase_switch(const FSR& fsr) {
...
void HTWKWalk::update_velocities(float swing_spline_value) {
    if (support_foot > 0) {
...
LegJoints HTWKWalk::set_joints(float waddle_right, float waddle_left, const AnkleBalancer& ankle_balancer,
...
float HTWKWalk::deadband_filter(float min, float value, float max){
        if(value<0) {
            value-=min;
            if(value>0) {
                value=0;
...
LegJoints HTWKWalk::proceed(const FSR& fsr, float body_pitch, float body_roll, const AnkleBalancer& ankle_balancer, float yaw,
